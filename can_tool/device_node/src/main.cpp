             ...
             uint8_t d7 = 0x00)
{
    ...
    bool res = (mcp2515.sendMessage(&msg) == MCP2515::ERROR_OK) ? true : false;
    if(res){
        printf("Sent: ");
        print_msg(msg);
    } else {
...
void irqHandler()
{
    ...
    if (irq & MCP2515::CANINTF_RX0IF)
    {
        if ((mcp2515.readMessage(MCP2515::RXB0, &read_frame) == MCP2515::ERROR_OK))
        {
            ...
            {
                printf("Requested: ");
                CanSend(0x2a0, false, 3, 0xa1, 0xa2, 0xa3);
            }
            else
            {
                printf("Got: ");
...
void setup()
{
    ...
    Serial.println("Example: CAN Device");
}

void loop()
{
    // uint8_t val = static_cast<uint8_t>(rand()&0xFF);
    // CanSend(0x2a2, false, 1, val);
    // delay(750);
    // CanSend(0x2a0, true);
    // delay(500);
}
