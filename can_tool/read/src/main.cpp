...
void loop()
{
    ...
    {
        print_msg(canMsg);
        if (addr == canMsg.data[CAN_DATA_ADDR]){
            uint8_t cmd = canMsg.data[CAN_DATA_CMD] & 0b11000000;
            uint8_t reg = canMsg.data[CAN_DATA_CMD] & 0b00111111;
            if (cmd == CAN_CMD_WRITE){
                printf("Write data to 0x%x reg\n\r", reg);
            } else if (cmd == CAN_CMD_READ)
            {
                printf("Read data from 0x%x reg\n\r", reg);
                CanSendRawMsg(prior, 2, 0xca, 0xfe);
            }
